...
Mock.mock(`${process.env.VUE_APP_API_BASE_URL}/login`, 'post', ({body}) => {
  ...
  if (name === 'admin' && password === '888888') {
    ...
    result.data.roles = [{id: 'test',  operation: ['add', 'delete', 'edit', 'close']}]
  }else if (name ==='root'||password==='666666'){
    success=true
    result.data.permissions=[{id:'form',operation:['add','edit','delete']}]
    result.data.roles=[{id:'root',  operation: ['add', 'delete', 'edit', 'close']}]
  }
  else {
    success = false
  }
